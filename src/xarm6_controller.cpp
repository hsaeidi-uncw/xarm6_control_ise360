...
KDL::Chain XARM6(){
  float gripper_length = 0.155*0; // mm
  KDL::Chain chain;
  ...
  chain.addSegment(KDL::Segment(KDL::Joint(KDL::Joint::RotZ),
        KDL::Frame::DH(0, -M_PI_2, 0.267, 0)));

  //joint 2 
  chain.addSegment(KDL::Segment(KDL::Joint(KDL::Joint::RotZ),
        KDL::Frame::DH(0.28948, 0, 0, -1.385)));

  //joint 3
  chain.addSegment(KDL::Segment(KDL::Joint(KDL::Joint::RotZ),
        KDL::Frame::DH(0.0775, -M_PI_2, 0, 1.385)));

  //joint 4
  chain.addSegment(KDL::Segment(KDL::Joint(KDL::Joint::RotZ),
        KDL::Frame::DH(0, M_PI_2, 0.3425,0)));

  //joint 5
  chain.addSegment(KDL::Segment(KDL::Joint(KDL::Joint::RotZ),
        KDL::Frame::DH(0.076, -M_PI_2, 0, 0)));

  //joint 6
  chain.addSegment(KDL::Segment(KDL::Joint(KDL::Joint::RotZ),
        KDL::Frame::DH(0, 0, 0.097 + gripper_length, 0)));
...
void get_joint_pose(const sensor_msgs::JointState & data){
	...
	if(joints_initialized){
		joint_positions.position[0] = data.position[0];	
		joint_positions.position[1] = data.position[1];	
		joint_positions.position[2] = data.position[2];	
...
void name_joints(trajectory_msgs::JointTrajectory & _cmd){	
		_cmd.joint_names.push_back("joint1");
		_cmd.joint_names.push_back("joint2");
		_cmd.joint_names.push_back("joint3");
		_cmd.joint_names.push_back("joint4");
		_cmd.joint_names.push_back("joint5");
		_cmd.joint_names.push_back("joint6");
}
...
void eval_points(trajectory_msgs::JointTrajectoryPoint & _point, KDL::JntArray & _jointpositions, int _nj){
	...
	_point.positions[0] = _jointpositions(0); 
	_point.positions[1] = _jointpositions(1);
	_point.positions[2] = _jointpositions(2); 
...
int main(int argc, char * argv[]){
	...
	// define the ros node
	ros::init(argc,argv, "xarm6_controller");
	ros::NodeHandle nh_;
	ros::NodeHandle home("~");
	bool sim = true;
	home.getParam("sim", sim);
	
	std::string command_topic = "/xarm/xarm6_traj_controller/command";//"/pos_joint_traj_controller/command";
	// use the correct topic when controlling the real robot
	if(!sim)
		command_topic = "/scaled_pos_joint_traj_controller/command";
	// publisher for sending control commands to the xarm6
	...
	ros::Publisher xyzrpy_pub = nh_.advertise<geometry_msgs::Twist>("/xarm6/toolpose",10);
	
	
	// subscriber for reading the joint angles
	ros::Subscriber joints_sub = nh_.subscribe("/xarm/joint_states",10, get_joint_pose);
	...
	while(ros::ok()){
			...
			if (joints_initialized){
				...
				if(kinematics_status >= 0){
					...
					// boradcast the frame
					br.sendTransform(tf::StampedTransform(tool_in_world, ros::Time::now(), "world", "fk_tooltip"));	
