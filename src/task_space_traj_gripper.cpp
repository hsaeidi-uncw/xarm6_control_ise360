...
xarm6_control_ise360::Plan plan;
xarm6_control_ise360::Plan prev_plan;
...
void get_plan(const xarm6_control_ise360::Plan & _data){
	plan = _data;
	plan_available = true;
}
...
bool is_new_plan(xarm6_control_ise360::Plan _plan, xarm6_control_ise360::Plan _prev_plan){
...
int main(int argc, char * argv[])
{
    ...
    ros::Publisher traj_status_pub = nh_.advertise<std_msgs::Bool>("/traj_expired",1);

    ros::Subscriber plan_sub = nh_.subscribe("/plan",10,get_plan);
    ros::Subscriber pos_sub = nh_.subscribe("/xarm6/toolpose" ,1, get_pos);
